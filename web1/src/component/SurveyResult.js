...
const SurveyResult = ({
  ...
  const needsSupportAreas = processed.filter(p => 
    p.included && p.stdScore < 40 && typeof p.stdScore === 'number' && !isNaN(p.stdScore)
  );
          ...
          <Box key={index} sx={{ mb: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              ...
              <Box sx={{ textAlign: 'right' }}>
                <Typography variant="h6" sx={{ fontWeight: 'bold', color: barColor }}>
                  {score}점
                </Typography>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 0.5, px: 0.5 }}>
              ...
              <Typography variant="caption" sx={{ color: '#333', fontWeight: 'bold' }}>50</Typography>
              <Typography variant="caption" sx={{ color: 'text.secondary' }}>75</Typography>
          ...
          {needsSupportAreas.length > 0 && (
            <Paper elevation={2} sx={{ p: { xs: 2, sm: 4 }, mt: 4, borderLeft: '4px solid #ff9800' }}>
              ...
              <Typography variant="body2" sx={{ mb: 3, color: 'text.secondary' }}>
                고위험 집단으로 적극적인 지원이 필요한 영역에 대한 전문 지원 서비스를 안내해드립니다.
              </Typography>
                ...
                return (
                  <Box key={area.key} sx={{ mb: 4, p: 3, backgroundColor: '#f9f9f9', borderRadius: 2 }}>
                    <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 2, color: 'primary.dark' }}>
                      {area.label} - {support.title} (점수: {Math.round(area.stdScore)}점)
                    </Typography>
